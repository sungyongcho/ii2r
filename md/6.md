...
---
__결론__: 1의 값은 어떤 값이 들어가도 상관이 없다. deltaDistX, deltaDistY (__광선에 진행에 따라 증가하게되는 '선'의 길이__)는 결국 벽(이나 피사체)의 '플레이어의 위치'로 부터 상대적 거리를 나타내기 위한 거리로 사용되고, 이 거리들은 각각의 거리가 다르다는 '비율'만 확실히 하게되면, |v|를 대신하는 1의 경우 어떤값이 들어가게 되도 상관없다.
...
빨간 박스의 부분에서 rayDir가 항상 1로 고정이 되어있다는 부분인데, rayDir가 항상 1이 된다는 부분에 조심 해야하는게, 벽 과 벽 사이의 거리인 rayDirX, rayDirY와 혼동이 되어서는 안된다!! 결국에는 비율이 중요한데,
<p align="center">
