...
광선을 구하는것 까지는 알겠는데, deltaDistX나 deltaDistY의 수식이 우리가 쉽게 알고 있는 피타고라스의 정의 등을 사용하면 될 것 같은데, 벡터를 사용하게 된 이유는, 삼각함수를 사용하게 될 경우 1.소숫점을 포함하게 되면서 계산이 느려질 수도 있고 (정리필요함 **확실하지않음**), 2. 벡터로 변경하는 것이 눈으로 보기에도 쉽다. 그래서 이 식이 어떻게 변형 되는지에 대하여 증명한 것을 살펴보자.
